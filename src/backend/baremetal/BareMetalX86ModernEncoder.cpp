#include "BareMetalX86ModernEncoder.hpp"
#include <cmath>

#define CALLEE_SAVED_GPR_COUNT 7
#define GPR_SAVE_BYTES (CALLEE_SAVED_GPR_COUNT * 8)

void BareMetalX86ModernEncoder::emitByte(uint8_t b) { 
    code.push_back(b); 
}

void BareMetalX86ModernEncoder::emitInt32(int32_t val) {
    uint8_t* p = reinterpret_cast<uint8_t*>(&val);
    for (int i = 0; i < 4; i++) emitByte(p[i]);
}

void BareMetalX86ModernEncoder::emitInt64(int64_t val) {
    uint8_t* p = reinterpret_cast<uint8_t*>(&val);
    for (int i = 0; i < 8; i++) emitByte(p[i]);
}

void BareMetalX86ModernEncoder::emitDouble(double val) {
    uint8_t* p = reinterpret_cast<uint8_t*>(&val);
    for (int i = 0; i < 8; i++) emitByte(p[i]);
}

void BareMetalX86ModernEncoder::emitREX(bool w, bool r, bool x, bool b) {
    uint8_t rex = 0x40;
    if (w) rex |= 0x08;
    if (r) rex |= 0x04;
    if (x) rex |= 0x02;
...
int BareMetalX86ModernEncoder::getStackOffset(const std::string& varName) {
...
void BareMetalX86ModernEncoder::emitStackLoad(uint8_t reg, int offset) {
    bool isExt = (reg >= 8);
    uint8_t r = reg & 7;
...
void BareMetalX86ModernEncoder::emitStackStore(uint8_t reg, int offset) {
    bool isExt = (reg >= 8);
    uint8_t r = reg & 7;
    emitREX(true, isExt, false, false);
    if (offset >= -128 && offset <= 127) {
...
void BareMetalX86ModernEncoder::emitValueLoad(uint8_t targetReg, const SSAValue& val) {
    if (val.isConstant) {
...
void BareMetalX86ModernEncoder::emitValueStore(uint8_t srcReg, const std::string& varName) {
    emitStackStore(srcReg, getStackOffset(varName));
}

void BareMetalX86ModernEncoder::emitXmmLoad(uint8_t xmmReg, int offset) {
...
void BareMetalX86ModernEncoder::emitXmmStore(uint8_t xmmReg, int offset) {
...
void BareMetalX86ModernEncoder::emitXmmValueLoad(uint8_t xmmReg, const SSAValue& val) {
    if (val.isConstant) {
...
void BareMetalX86ModernEncoder::emitXmmValueStore(uint8_t xmmReg, const std::string& varName) {
...
void BareMetalX86ModernEncoder::encodeInstruction(const SSAInstruction& inst) {
    switch (inst.op) {
        case SSAOp::Add:
        case SSAOp::Sub:
        case SSAOp::Mul: {
        ...
        case SSAOp::VolatileLoad: {
            emitValueLoad(0, inst.operands[0]); 
            if (inst.result.type == LsmStaticType::Int16) {
                emitByte(0x0F); emitByte(0xB7); emitByte(0x00); 
            } else if (inst.result.type == LsmStaticType::Int32) {
                emitByte(0x8B); emitByte(0x00);                 
            } else if (inst.result.type == LsmStaticType::Int64 || inst.result.type == LsmStaticType::Ptr) {
        ...
        case SSAOp::VolatileStore: {
            emitValueLoad(0, inst.operands[0]); 
            emitValueLoad(1, inst.operands[1]); 
            if (inst.operands[1].type == LsmStaticType::Int16) {
                emitByte(0x66); emitByte(0x89); emitByte(0x08); 
            } else if (inst.operands[1].type == LsmStaticType::Int32) {
                emitByte(0x89); emitByte(0x08);                 
            } else if (inst.operands[1].type == LsmStaticType::Int64 || inst.operands[1].type == LsmStaticType::Ptr) {
            ...
            for (size_t i = 1; i <= maxRegArgs; ++i) {
                ...
                if (inst.operands[i].type == LsmStaticType::Float64) {
                    ...
                    if (argIdx < 4) {
                        ...
                        emitValueLoad(0, inst.operands[i]);
                        emitREX(true, false, false, true); emitByte(0x89); emitByte(0xC8);
                    }
                }
            }
...
void BareMetalX86ModernEncoder::encodeFunctionInternal(const SSAFunction& func) {
    stackOffsets.clear();
    ...
    if (!func.isNaked) {
        ...
        int localVarsBytes = std::abs(currentStackOffset);
        int dynamicStackSize = (localVarsBytes + 15) & ~15;
        if (dynamicStackSize < 4096) dynamicStackSize = 4096;

        emitREX(true, false, false, false); emitByte(0x81); emitByte(0xEC); emitInt32(dynamicStackSize); 
...
std::vector<uint8_t> BareMetalX86ModernEncoder::encodeProgram(const std::vector<SSAFunction>& program) {
    code.clear();
    blockOffsets.clear();
    jumpPatches.clear();
